...
parser.add_argument("--gpu", type=int, default=None, help='GPU device id to use (default: auto-detect, use -1 for CPU)')
...
def main():
    ...
    if opt.gpu == -1:
        ...
        print('Forced to use CPU (--gpu -1)\n')
    elif opt.gpu is not None:
        # Use specific GPU
        if torch.cuda.is_available():
            if opt.gpu >= torch.cuda.device_count():
                print(f'Warning: GPU {opt.gpu} not available. Using GPU 0 instead.')
                opt.gpu = 0
        ...
        if torch.cuda.is_available():
            ...
            print(f'CUDA available. Using GPU 0: {torch.cuda.get_device_name(0)}\n')
    ...
    # Helper function to remove 'module.' prefix from state_dict keys
    ...
    if device.type == 'cuda':
        gpu_id = opt.gpu if opt.gpu is not None and opt.gpu >= 0 else 0
        device_ids = [gpu_id]
        model = nn.DataParallel(net, device_ids=device_ids).to(device)
    else:
        model = net.to(device)
    state_dict = torch.load(os.path.join(opt.delog, 'net.pth'), map_location=device)
    ...
    if opt.mode == "MC":
        if device.type == 'cuda':
            gpu_id = opt.gpu if opt.gpu is not None and opt.gpu >= 0 else 0
            device_ids = [gpu_id]
            model_est = nn.DataParallel(est_net, device_ids=device_ids).to(device)
        else:
            model_est = est_net.to(device)
        est_state_dict = torch.load(os.path.join(opt.delog, 'est_net.pth'), map_location=device)
    ...
    test_data_path = os.path.join('data', opt.test_data)
    print(f'Looking for files in: {test_data_path}')
    files_source = glob.glob(os.path.join('data', opt.test_data, '*.*'))
    files_source.sort()
    print(f'Found {len(files_source)} file(s)')
    if len(files_source) == 0:
        print(f'No files found! Please check the path: {os.path.abspath(test_data_path)}')
        return
    # process data
    psnr_test = 0
    ...
    for f in files_source:
        ...
        ISource, INoisy, True_Res = Variable(ISource.to(device),volatile=True), Variable(INoisy.to(device),volatile=True), Variable(True_Res.to(device),volatile=True)
        ...
        if (opt.ps == 1 or opt.ps == 2) and pss!=1:  #pixelshuffle multi-scale
            ...
            for row in range(pss):
                for column in range(pss):
                    ...
                    re_test_tensor = Variable(re_test_tensor.to(device),volatile=True)
        ...
        # Format k as integer if it's a whole number, otherwise as float
        k_str = str(int(k)) if k == int(k) else str(k)
        output_filename = file_name + '_pss'+str(pss)+'_k'+k_str+'.png'
        output_path = os.path.join(opt.out_dir, output_filename)
        print(f'Saving output to: {os.path.abspath(output_path)}')
        cv2.imwrite(output_path, merge_out_numpy[:,:,::-1])
