...
def decide_scale_factor(noisy_image, estimation_model, color=1,  thre = 0, plot_flag = 1, stopping = 4, mark=''):
    ...
    Pre_CDF = None
    flag = 0
    # Get device from estimation_model
    device = next(estimation_model.parameters()).device
    for pss in range(1, stopping+1):  #scaling factor from 1 to the limit
        ...
        INoisy = Variable(INoisy.to(device), volatile=True)
...
def get_smooth_maps(lm, dilk = 50, gsd = 10):
    ...
    device = lm.device if hasattr(lm, 'device') else torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    RF_tensor = Variable(RF_tensor.to(device),volatile=True)
    return RF_tensor
...
def level_refine(NM_tensor, ref_mode, chn=3):
    ...
    if ref_mode == 0 or ref_mode == 1 or ref_mode == 4 or ref_mode==5:  #if we use a single value for the map
        ...
        device = NM_tensor.device if hasattr(NM_tensor, 'device') else torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        RF_tensor = Variable(RF_tensor.to(device),volatile=True)
...
def generate_denoise(image, model, noise_level_list):
    ...
    ISource = Variable(ISource.to(device),volatile=True)
