...
def train(config):
    ...
    train_loader = torch.utils.data.DataLoader(train_dataset,
                                               ...
                                               shuffle=True,
                                               num_workers=0,     # ⚠ M1 建议=0 否则多进程容易卡住
                                               pin_memory=False)  # 无 CUDA 用不到 pin_memory

    # Loss
    L_color = Myloss.L_color().to(device)
    L_spa   = Myloss.L_spa(device=device).to(device)
    L_exp   = Myloss.L_exp(16, device=device).to(device)
    ...
    for epoch in range(config.num_epochs):
        for iteration, img_lowlight in enumerate(train_loader):
            ...
            loss = (1600 * L_TV(A)
                    +      torch.mean(L_spa(enhanced_image, img_lowlight))
                    + 5  * torch.mean(L_color(enhanced_image))
                    + 10 * torch.mean(L_exp(enhanced_image, 0.6)))
        ...
        # 每个 epoch 保存模型一次（更合理）
        torch.save(DCE_net.state_dict(), f"{config.snapshots_folder}/Epoch{epoch}.pth")
        print(f"✔ Saved ==> Epoch{epoch}.pth")
